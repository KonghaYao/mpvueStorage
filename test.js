...
fetch("https://xiumi.us/api/shows/all/tags?team_id=321738", { headers: { cookie }, referrer: "https://xiumi.us/" })
    .then((res) => res.json())
    .then((res) => {
        let allArticle = {};
        for (const tag of res.data) {
            await getArticleByTag(tag)
        }
    });
function getArticleByTag(tag) {
    return fetch(`https://xiumi.us/api/shows/by/tags/${tag}?show_type&team_id=321738&limit=20&page=0`, {
        headers: {
            accept: "application/json, text/plain, */*",
            "accept-language": "zh-CN,zh;q=0.9,en;q=0.8",
            "cache-control": "no-cache",
